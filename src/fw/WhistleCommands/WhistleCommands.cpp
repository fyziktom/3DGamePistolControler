...
static bool g_enable_periodic_serial_debug = true;
static bool g_enable_graphs                = true;
static bool g_enable_overlay               = true;
static bool g_own_display                  = true;
static bool g_call_m5_update               = true;
static bool g_init_serial                  = true;
static uint32_t g_serial_baud              = 115200;
static LGFX_Device* g_display              = nullptr;
...
static const WhistleCommandDef DEFAULT_COMMANDS[] = {
...
static const WhistleCommandDef* g_command_defs   = DEFAULT_COMMANDS;
static size_t                   g_command_count  = sizeof(DEFAULT_COMMANDS) / sizeof(DEFAULT_COMMANDS[0]);

class ToneSequencer {
public:
  void begin(const WhistleCommandDef* cmds, size_t cmd_count) {
    _cmds = cmds;
  ...
  void setUserCallback(WhistleCommandCallback cb) { _user_cb = cb; }
...
private:
  ...
  const WhistleCommandDef* _cmds = nullptr;
  size_t _cmd_count = 0;
  WhistleCommandCallback _user_cb = nullptr;
  ...
  void periodicDebug(uint32_t now_ms) {
    if (!g_enable_periodic_serial_debug) return;
  ...
  void evaluateSequenceIfEnded(uint32_t now_ms) {
    ...
    const WhistleCommandDef* best = nullptr;
    uint8_t best_len = 0;

    for (size_t ci = 0; ci < _cmd_count; ++ci) {
      const WhistleCommandDef& cmd = _cmds[ci];
    ...
    if (best) {
      ...
      if (_user_cb) {
        _user_cb(best->id, best->name, best->pattern, midi_tail, freq_tail, best_len);
      }
    } else {
...
static void drawOverlay() {
  if (!g_enable_overlay || g_display == nullptr) return;
...
// Setup
// -----------------------------------------------------------------------------
bool WhistleCommands::begin(const WhistleCommandsConfig& cfg) {
  g_enable_periodic_serial_debug = cfg.enablePeriodicSerialDebug;
  g_enable_graphs                = cfg.enableGraphs;
  g_enable_overlay               = cfg.enableOverlay;
  g_own_display                  = cfg.ownDisplay;
  g_call_m5_update               = cfg.callM5Update;
  g_init_serial                  = cfg.initSerial;
  g_serial_baud                  = cfg.serialBaud;

  g_display = cfg.display ? cfg.display : static_cast<LGFX_Device*>(&M5.Display);
  if (g_display == nullptr) {
    g_enable_graphs  = false;
    g_enable_overlay = false;
    g_own_display    = false;
  }

  g_command_defs  = (cfg.commands && cfg.commandCount > 0) ? cfg.commands : DEFAULT_COMMANDS;
  g_command_count = (cfg.commands && cfg.commandCount > 0)
                    ? cfg.commandCount
                    : sizeof(DEFAULT_COMMANDS) / sizeof(DEFAULT_COMMANDS[0]);

  if (!cfg.skipM5Begin) {
    if (g_init_serial) {
      Serial.begin(g_serial_baud);
      delay(100);
      Serial.println();
  ...
  tone_seq.setUserCallback(cfg.onCommand);

  // Use landscape orientation.
  if (g_display && g_own_display && (g_enable_graphs || g_enable_overlay)) {
    g_display->setRotation(cfg.displayRotation);
    g_display->setBrightness(cfg.displayBrightness);
    g_display->fillScreen(0x000000U);
  }
  ...
  if (!M5.Mic.isEnabled()) {
    if (g_display && g_own_display) {
      g_display->setCursor(0, 0);
      g_display->setTextColor(0xFFFF00U, 0x000000U);
      g_display->println("Microphone not available");
    }
    return false;
  }
  ...
  int16_t w = g_display ? g_display->width() : 0;
  int16_t h = g_display ? g_display->height() / 3 : 0;
  int16_t y = 0;
  ...
  if (g_enable_graphs && g_display) {
    fft_drawer.setup(g_display, rect_fft_drawer);
    fft_history.setup(g_display, rect_fft_history);
    wav_drawer.setup(g_display, rect_wav_drawer);
  }

  if (g_display && g_own_display && (g_enable_graphs || g_enable_overlay)) {
    g_display->setFont(&fonts::AsciiFont8x16);
  ...
  tone_seq.begin(g_command_defs, g_command_count);
...
void WhistleCommands::update() {
  ...
  if (M5.Mic.isEnabled()) {
    ...
    switch (++step) {
      ...
      case 0:
        step = 0;
        if (g_display && g_enable_graphs && g_own_display) {
          g_display->display();
        }
...
int WhistleCommands::lastCommandId() const {
  return tone_seq.lastCommandId();
}
